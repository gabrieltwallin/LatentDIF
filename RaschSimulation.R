...
setwd("C:/Users/gawa0005/Dropbox/Psychometrics/VR project/Cheating detection/Code/LatentDIF/Simulation Results")
...
mse_sd <- c()
mse_pi <- c()
mse_dif <- matrix(0, nrow = J, ncol = sim)
...
for(j in 1:sim){
  ...
  line.search <- function(data, a.vec, d.vec, delta.vec, pi, mu, sigma, grad.a, grad.d, grad.delta, grad.sigma, grad.mu, lambda, step=1) {
  ...
  Update.par <- function(post, data, a.vec, d.vec, delta.vec,
    ...
    step <- line.search(a.vec, d.vec, delta.vec, pi, mu, sigma, grad.a, grad.d, 
  ...
  Update.par_conf <- function(data, post, data, a.vec, d.vec, delta.vec, 
                              mu, sigma, step){
  ...
  EM <- function(data, a.vec, d.vec, delta.vec, pi, mu, sigma, lambda, step){
    ...
    for(i in 1:1){
      res = Update.par(post, data, a.vec, d.vec, delta.vec, mu, sigma, 
                       lambda, step)
      a.vec = res$a.vec
      d.vec = res$d.vec
  ...
  EM_conf <- function(data, a.vec, d.vec, delta.vec, pi, mu, sigma, step){ # I had 'step' as an argument before
    ...
    bic=-2*llik(data, a.vec, d.vec, delta.vec, pi, mu, sigma) + (J+J+3 + sum(delta.vec!=0))
    
    list(a.vec = a.vec, d.vec = d.vec, delta.vec = delta.vec, mu = mu, sigma = sigma, pi = pi, post = post, bic = bic)
  }
  
  
  delta.vec0 = delta.vec+rnorm(J, 0, 0.1) # Previously N(0, 0.05)
  
  lambda.vec <- seq(0.01, 0.0005, by = -0.001)
  
  res = EM(data, a.vec, d.vec, delta.vec0, pi, mu, sigma, lambda = lambda.vec[1], step)
  ...
  # MSE
  mse_mean[j] <- (1/K)*(mu - bias_mean[j])^2
  mse_sd[j] <- (1/K)*(sigma - bias_sd[j])^2
  mse_pi[j] <- (1/K)*(pi - bias_pi[j])^2
  mse_dif[,j] <- (1/J)*(delta.vec - bias_dif[,j])^2
  mse_d.vec[,j] <- (1/J)*(d.vec - bias_d.vec[,j])^2
  mse_a.vec[,j] <- (1/J)*(a.vec - bias_a.vec[,j])^2
...
#    for (i in 1:100) {
#      threshold <- thresholds[i]
###      predicted <- ifelse(class.assign_tmp > threshold, 1, 0)
  #    tp <- sum(predicted == 1 & xi.vec == 1)
#      fp <- sum(predicted == 1 & xi.vec == 0)
#      fn <- sum(predicted == 0 & xi.vec == 1)
#      tn <- sum(predicted == 0 & xi.vec == 0)
#      fpr_est[i] <- fp / (fp + tn)
#      tpr_est[i] <- tp / (tp + fn)
#    }
    
 #   roc_curves_est[[j]] <- data.frame(fpr_est, tpr_est)
 ...
 #   thresholds <- seq(0, 1, length.out = 100)
#    fpr_true <- numeric(100)
#    tpr_true <- numeric(100)
    
#    for (i in 1:100) {
#      threshold <- thresholds[i]
#      predicted <- ifelse(class.assign_tmp_true > threshold, 1, 0)
#      tp <- sum(predicted == 1 & xi.vec == 1)
#      fp <- sum(predicted == 1 & xi.vec == 0)
#      fn <- sum(predicted == 0 & xi.vec == 1)
#      tn <- sum(predicted == 0 & xi.vec == 0)
...
#    }
 ...
 #   roc_curves_true[[j]] <- data.frame(fpr_true, tpr_true)
